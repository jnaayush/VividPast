...
L_images = np.load("LAB.np")
[count, r, c, chan] = L_images.shape
...
for i in range(0, 1):
    ...
    for j in range(i*batch + 1, i*batch+100):
        print("RANGE: " + str(j))
        img_L = L_images[j, :, :]
        img = np.stack((img_L, img_L, img_L), axis = 2)
        w, h, _ = img.shape
    ...
    #np.save("results_train_" + str(i*batch) + "to" + str(i*batch+500) + ".npy", results_train)
