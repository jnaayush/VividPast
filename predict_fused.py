...
from training_utils import  fused_evaluation_pipeline, checkpointing_system, maybe_create_folder
...
def norm_data(l, ab, seg):
    ...
    seg_data = (seg / factors["seg_factor"]).astype('float32')
    
    return L_channel, AB_channel, seg_data

##### LOAD DATA ####

l, ab, seg = norm_data(l_array, ab_array, seg_array)

saver, checkpoint_paths, latest_checkpoint = checkpointing_system(run_id)
evaluation = fused_evaluation_pipeline(colorizer, l, seg, ab)
